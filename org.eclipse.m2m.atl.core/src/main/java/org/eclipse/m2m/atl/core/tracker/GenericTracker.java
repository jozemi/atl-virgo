...
import org.eclipse.m2m.atl.core.tracker.wrapper.ModelWrapper;
...
public class GenericTracker {
	...
	private Map<String, IModelWrapper> modelTracker = new HashMap<String, IModelWrapper>();
	 ...
	 * @param properties
	 */
	public void register(IModelWrapper im) {
		String name = im.getName();

		if (!modelTracker.containsKey(name)) {
			modelTracker.put(name, im);
			LOG.debug("Element {1} registered", name);
		} else {
			LOG.error("Element {1} previously registered", name);
		}
	}
	...
	public void unRegisterModel(IModelWrapper im) {
		String name = im.getName();

		if (modelTracker.containsKey(name)) {
			modelTracker.remove(name);
			LOG.debug("Element {1} unregistered", name);
		} else {
			LOG.error("Element {1} wasn not previously registered", name);
		}
	}
	...
	public Object getClass(String extensionId, String executableExtensionName, String extensionName) {
		...
		if (extensionId.equals(MODELS_EXTENSION_POINT)) {
			IModelWrapper imw = modelTracker.get(extensionName);
			if (imw != null) {
				if (executableExtensionName.equals("modelFactory")) {
					try {
						Class c = Class.forName(imw.getModelFactory());
						return c.newInstance();
					} catch (Exception e) {
						LOG.error("Creating objet", e);
					}
				}
			} else {
